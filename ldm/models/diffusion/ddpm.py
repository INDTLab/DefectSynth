...
from ldm.models.autoencoder_con import IdentityFirstStage, AutoencoderKL
...
class DDPM(pl.LightningModule):
    def __init__(self, unet_config, timesteps=1000, beta_schedule="linear", loss_type="l2", ckpt_path=None, ignore_keys=[], load_only_unet=False, monitor="val/loss", use_ema=True, first_stage_key="image", image_size=256, channels=3, log_every_t=100, clip_denoised=True, linear_start=1e-4, linear_end=2e-2, cosine_s=8e-3, given_betas=None, original_elbo_weight=0., embedding_reg_weight=0., unfreeze_model=False, model_lr=0., v_posterior=0., l_simple_weight=1., conditioning_key=None, parameterization="eps", scheduler_config=None, use_positional_encodings=False, learn_logvar=False, logvar_init=0.,):
        super().__init__()
        ...
        self.register_schedule(given_betas=given_betas, beta_schedule=beta_schedule,
                               timesteps=timesteps, linear_start=linear_start, linear_end=linear_end, cosine_s=cosine_s)
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        missing, unexpected = self.load_state_dict(
            sd, strict=False)if not only_model else self.model.load_state_dict(sd, strict=False)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self, first_stage_config, cond_stage_config, personalization_config, num_timesteps_cond=None, cond_stage_key="image", cond_stage_trainable=False, concat_mode=True, cond_stage_forward=None, conditioning_key=None, scale_factor=1.0, scale_by_std=False, *args, **kwargs):
    ...
    def prepare_spatial_encoder(self, optimze_together=False, data_enhance=False, l=4):
        ...
        if data_enhance:
            from utils.augment import random_transform
    ...
    def get_learned_conditioning(self, c, x=None, name=None):
        ...
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                c, position = self.cond_stage_model.encode(
                    c, cond_img=x, embedding_manager=self.embedding_manager, name=name)
    ...
    def delta_border(self, h, w):
        ...
        :param h: height
        :param w: width
        :return: normalized distance to image border, wtith min distance = 0 at border and max dist = 0.5 at image center
        """
        ...
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ...
                if isinstance(self.first_stage_model, VQModelInterface):
                    output_list = [self.first_stage_model.decode(z[:, :, :, :, i], force_not_quantize=predict_cids or force_not_quantize)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, mask=None, weight_map=None, **kwargs):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation", 'bbox_img'] and self.model.conditioning_key:
                ...
                tl_patch_coordinates = [(rescale_latent * stride[0]*(patch_nr % n_patches_per_row)/full_img_w, rescale_latent * stride[1]*(patch_nr // n_patches_per_row)/full_img_h)
                                        for patch_nr in range(z.shape[-1])]

                patch_limits = [(x_tl, y_tl, rescale_latent * ks[0]/full_img_w, rescale_latent *
                                 ks[1]/full_img_h)for x_tl, y_tl in tl_patch_coordinates]
    ...
    @torch.no_grad()
    def p_sample(self, x, c, t, clip_denoised=False, repeat_noise=False, return_codebook_ids=False, quantize_denoised=False, return_x0=False, temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance(x=x, c=c, t=t, clip_denoised=clip_denoised, return_codebook_ids=return_codebook_ids,
                                       quantize_denoised=quantize_denoised, return_x0=return_x0, score_corrector=score_corrector, corrector_kwargs=corrector_kwargs)
        if return_codebook_ids:
    ...
    @torch.no_grad()
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False, img_callback=None, mask=None, x0=None, temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, batch_size=None, x_T=None, start_T=None, log_every_t=None):
        if not log_every_t:
        ...
        for i in iterator:
            ...
            img, x0_partial = self.p_sample(img, cond, ts, clip_denoised=self.clip_denoised, quantize_denoised=quantize_denoised, return_x0=True,
                                            temperature=temperature[i], noise_dropout=noise_dropout, score_corrector=score_corrector, corrector_kwargs=corrector_kwargs)
            if mask is not None:
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, unconditional_only=False, ddim_steps=200, ddim_eta=1., return_keys=None, quantize_denoised=True, inpaint=True, plot_denoise_rows=False, plot_progressive_rows=False, plot_diffusion_rows=False, adaptive_mask=False, **kwargs):
        use_ddim = ddim_steps is not None
        log = dict()
        z, c, total_dict, x, xrec, xc = self.get_input(
            batch, self.first_stage_key, return_first_stage_outputs=True, force_c_encode=True, return_original_cond=True, bs=N)
        ...
        if sample:
            ...
            sample_scaled, _ = self.sample_log(cond=c, batch_size=N, ddim=use_ddim, ddim_steps=ddim_steps,
                                               eta=ddim_eta, unconditional_guidance_scale=5.0, unconditional_conditioning=uc)
        ...
        if inpaint:
            ...
            with self.ema_scope("Plotting Inpaint"):
                samples, z_denoise_row = self.sample_log(cond=c, batch_size=N, ddim=use_ddim, eta=ddim_eta, ddim_steps=ddim_steps, x0=z[:N], mask=mask, embedding_position=total_dict[
                                                         'position'], adaptive_mask=adaptive_mask, embedding_manager=self.cond_stage_model, decoder=self.decode_first_stage)
    ...
    def configure_optimizers(self):
        ...
        if self.embedding_manager is not None:
            ...
            if self.unfreeze_model:
                ...
                opt = torch.optim.AdamW([{"params": embedding_params, "lr": lr}, {
                                        "params": model_params}], lr=self.model_lr)
            else:
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if os.path.isdir(self.trainer.checkpoint_callback.dirpath):
            if self.spatial_encoder:
                torch.save(self.embedding_manager.spatial_encoder_model.state_dict(), os.path.join(
                    self.trainer.checkpoint_callback.dirpath, "spatial_encoder.pt"))
            self.embedding_manager.save(os.path.join(self.trainer.checkpoint_callback.dirpath, "embeddings.pt"))
            self.embedding_manager.save(os.path.join(self.trainer.checkpoint_callback.dirpath,
